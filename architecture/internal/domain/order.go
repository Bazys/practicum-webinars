// Пакет полностью изолирован. Мы можем взять этот пакет
// и перенести в любой другой проект.
// В нем нет import "net/http", нет import "database/sql".
// Это и есть чистый домен.
// Валидация (quantity <= 0) теперь защищает нас
// от создания некорректных сущностей на уровне конструктора.
package domain
